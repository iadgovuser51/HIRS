...
CA_CERT_DIR_ESCAPED=\\/var\\/data/\\/hirs\\/certificates
...
if ! [ -f $CA_PEM ]; then
    ...
    chmod -R 775 ${HIRS_DIR}/certificates/*
    chmod 775 ${HIRS_DIR}/certificates/private
    chmod -R 775 ${HIRS_DIR}/certificates/private/*
...
if [[ $1 = "server" ]]; then
    ...
    if [[ -z `grep -o "keystoreFile=\"${CA_CERT_DIR_ESCAPED}\/keyStore.jks\"" $CATALINA_HOME/conf/server.xml` ]]; then
        ...
        chown -R developer:developer ${HIRS_DIR}/certificates/

        # create an alias in the keystore for developer
        alias=$(keytool -list -v -keystore ${KEYSTORE_JKS} -storepass ${P12_PASSWORD} | grep -B2 'PrivateKeyEntry' | grep 'Alias name:')
        keytool -changealias -alias ${alias#*:} -destalias developer -v -keystore ${KEYSTORE_JKS} -storepass ${P12_PASSWORD}
...
EOF
        ...
        chgrp -R developer ${CATALINA_HOME}
...
if [[ $1 = "server" ]]; then
    ...
    if ! [[ -d $MYSQL_CERT_DIR ]]; then
        ...
        chgrp -R developer $MYSQL_CERT_DIR
